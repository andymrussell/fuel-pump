...
class Pagination
{
	...
	public static function page_links()
	{
		...
		for($i = $start; $i <= $end; $i++)
		{
			...
			{
				$url = ($i == 1) ? '' : '/'.$i;
				$pagination .= static::$template['active_start'].\Html::anchor(rtrim(static::$pagination_url, '/').$url.static::$template['append_link'], $i).static::$template['active_end'];
			}
			else
			{
				$url = ($i == 1) ? '' : '/'.$i;
				$pagination .= static::$template['pagelinks_start'].\Html::anchor(rtrim(static::$pagination_url, '/').$url.static::$template['append_link'], $i).static::$template['pagelinks_end'];
			}
		}
	...
	public static function next_link($value)
	{
		...
		else
		{
			...
			return static::$template['next_start'].\Html::anchor(rtrim(static::$pagination_url, '/').'/'.$next_page.static::$template['append_link'], $value.static::$template['next_mark']).static::$template['next_end'];
		}
	}
	...
	public static function prev_link($value)
	{
		...
		else
		{
			...
			return static::$template['previous_start'].\Html::anchor(rtrim(static::$pagination_url, '/').$previous_page.static::$template['append_link'], static::$template['previous_mark'].$value).static::$template['previous_end'];
		}
	}
}